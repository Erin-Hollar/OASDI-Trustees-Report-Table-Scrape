  ...
  #Year: 2014-2022
  #For eacg year, sections II through VI
  #For each section, subsectiond A thourgh H
  #For each subsection, table number 1 through 10
  Year_list <- 2014:2022
  ...
  # Generate data for each year, section, subsection, and table number
  ...
  #Add html bit columns by filtered for the section and subsection and manufally assinging the unique end of the html
  #Skip to line 180 
  #II.B
  ...
  #i.e. Is it the 1st graph on its webpage -> [1]
  main_table <- main_table %>%
  ...
  #Add full html bit column, referencing the other columns
  ...
  main_table_final <- main_table %>% filter (!is.na(table_name))#Remove observations that do not have a table
  main_table_final <- main_table_final[nrow(main_table_final):1, ] #reverse order of observations, for sorting preferences
  
  # test_table <- main_table_final %>%
  #   filter(Section == "VI" & Subsection == "G" )
  ...
  #add something about if year ==2014;2018, the first row is variables
  ...
  title <- link_list %>% html_nodes("div.TableTitle") %>% 
     html_text() %>%
     .[title_num]
  print(title)
  # table <- body%>%
  #   mutate(Year=Year[i])
   #  if (year == "2014"){ ##FIX THIS
   #    body%>%
   #    row_to_names(,row_number=2)
   # }
  rawdata[[section]][[subsection]][[number]][[year]][[title]] <- body
  
  # if (year == "2014" | year == "2015") {
  #   df<-rawdata[[section]][[subsection]][[number]][[year]][[title]][[1]]
  #   colnames(df)<-df[2,]
  #   df<-df[-2,]
  #   df
  # 
  ...
  #   section <- as.character(test_table$Section[i])
  #   subsection <- as.character(test_table$Subsection[i])
  #   number <- as.character(test_table$Table_Number[i])
  #   year <- as.character(test_table$Year[i])
  #   html <- test_table$html[i]
  #   table_name <- str_c("table." , test_table$table_name[i])
  #   title_num <- test_table$Title_Number[i]
  #   
  #   # Check if the 'section' exists in the rawdata list & create if not
  #   if (!(section %in% names(rawdata))) {
  #     rawdata[[section]] <- list()
